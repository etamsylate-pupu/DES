...
void ECB(const char* plaintext, const char* keytext, char** ciphertext) {
	...
	char* cipher;
	char hex[16];
	char temp[8],temp_char[8];
	*ciphertext= (char *)malloc((2*strlen(plaintext)+1)*sizeof(char)); //动态分配内存，记住加1，需要以NULL结尾
	//*ciphertext= (char *)malloc((strlen(plaintext)+1)*sizeof(char));
	cipher=*ciphertext;

	char_to_bool(bool_key,keytext,64); //将字节形式的密钥改为二进制形式
	num=(strlen(plaintext))/8;   //看几个分组进行加密
	for(i=0;i<num;i++){
		for(j=0;j<8;j++){
            temp[j]=plaintext[j+i*8];    //保存8字节的明文
		}
		char_to_bool(plain_set,temp,64);  //将字节形式的明文转为二进制形式
		Encrypt(cipher_set,plain_set,bool_key);
		bit_to_hex(hex,cipher_set,64); //将密文转为16进制数表示
		for(j=0;j<16;j++){
			cipher[i*16+j]=hex[j];
		}/*bool_to_char(temp_char,cipher_set,64);
		for(j=0;j<8;j++){
            cipher[i*8+j]=temp_char[j];
		}*/
	}
	cipher[2*strlen(plaintext)]=NULL;  //以NULL结尾
	//cipher[strlen(plaintext)]=NULL;
};
void CBC(const char* plaintext, const char* keytext, const char* vitext, char** ciphertext) {
    ...
    char* cipher;
	*ciphertext= (char *)malloc((2*strlen(plaintext)+1)*sizeof(char));
    cipher=*ciphertext;
	num=(strlen(plaintext))/8;//因为plaintext为字符表示，即字节所以需要计算明文分组数，分为num组
	char_to_bool(bool_key,keytext,64); //将字符型密钥转换为二进制数表示的密钥
	char_to_bool(bool_iv,vitext,64);
    Table_copy(bool_temp,bool_iv,64);
	for(i=0;i<num;i++){
		for(j=0;j<8;j++){
            temp[j]=plaintext[j+i*8];
		}
		char_to_bool(plain_set,temp,64);
	...
	cipher[strlen(plaintext)*2]=NULL;  //以NULL结尾
};
...
void CFB(const char* plaintext, const char* keytext, const char* vitext, char** ciphertext) {
	...
	*ciphertext= (char *)malloc((2*strlen(plaintext)+1)*sizeof(char));
	...
	num=(strlen(plaintext))/1; //为8位CFB模式
	char_to_bool(bool_key,keytext,64);
	char_to_bool(bool_iv,vitext,64);
...
void OFB(const char* plaintext, const char* keytext, const char* vitext, char** ciphertext) {
	...
	*ciphertext= (char *)malloc((2*strlen(plaintext)+1)*sizeof(char));
	...
	num=(strlen(plaintext))/1;//8位OFB模式
	char_to_bool(bool_key,keytext,64);
	char_to_bool(bool_iv,vitext,64);
	Table_copy(bool_temp,bool_iv,64);
	Table_copy(bool_temp1,bool_iv,64);
